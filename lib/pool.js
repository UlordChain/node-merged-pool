...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    */
    ...
    function SubmitUscBlock(params) {
        console.log("uscblock submit:");
        var blockM = new blockMake(params);
	//console.log('blockM', blockM);
        blockM.makeSubmitMsg(function(err, rawbm){
            ...
            if(isValidShare && !shareData.isDuplicate){
                if(shareData.isUscTarget){	
                    ...
                    var params = [
                        ...
                        String(shareData.merkleHashes.split(" ").length)  //blockTxnCountHex                                               
                    ]
            ...
            gwMaker.makeRawGwMsg(function(rawgw){
                if(!rawgw.notify){
                    if(getArr.indexOf(rawgw.blockHashForMergedMining) === -1){
                        ...
                        console.log('usc duplicate blocks');
                        data.isDuplicate = true;
                       data.uscData = undefined;
                    }
                }else{
                    getArr = [];
                    getArr.push(rawgw.blockHashForMergedMining);
    		    ...
    		    gwMaker.makeRawGwMsg(function(rawgw){
                    ...
                    if(!rawgw.notify){
                        if(getArr.indexOf(rawgw.blockHashForMergedMining) === -1){
                            ...
                            data.isDuplicate = false;
                            data.uscData = rawgw;
                            console.log('(new BlockhashForMM)notify-false;;;;;;;;;;;;;;;;;;;;');
                        }else{
                            console.log('usc duplicate blocks');
                            data.isDuplicate = true;
                            data.uscData = undefined;
