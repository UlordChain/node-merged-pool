...
//get usc tag asm
exports.getUscTag = function(string){
	var blockHash = string.substr(2);
	var tagBuffer = this.serializeString("USCBLOCK:"); 
	var blockHashBuffer = this.serializeString(new Buffer(blockHash,"hex"));    
	tagBuffer = tagBuffer.slice(1,tagBuffer.length);    
	blockHashBuffer =  blockHashBuffer.slice(1,blockHashBuffer.length);    
	var tag = Buffer.concat([ tagBuffer, blockHashBuffer ]);
	var tagLengthInHex = (tag.toString("hex").length/2).toString(16);
    //console.log('tag', this.hex2a(new Buffer(tagLengthInHex,"hex")));
	return Buffer.concat([new Buffer([0x6a, 0x4c]), new Buffer(tagLengthInHex,"hex"), tag]);
}
