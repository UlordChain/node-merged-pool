 ...
 **/
 var JobManager = module.exports = function JobManager(options){
	...
	if(rpcData.uscData){
		var uscTag = rpcData.uscData.blockHashForMergedMining;
	}else{
		var uscTag = '';
	}
    ...
    this.processSecondShare = function(jobId, previousDifficulty, difficulty, ipAddress, port, workerName, nTime, nonce, extraNonce1, extraNonce2, hash){
	  ...
	  if(_this.uscTag !== ''){
		var isUscTarget = job.uscTarget.ge(headerBigNum);	//true
	  }else{
		var isUscTarget = false;
	  }	
      var blockDiffAdjusted = job.difficulty * shareMultiplier;
	  blockHexInvalid = job.serializeBlock(headerBuffer).toString('hex');
	  //blockHashInvalid = blockHasher(headerBuffer, null).toString('hex');	
	  blockHashInvalid = util.reverseBuffer(util.sha256d(headerBuffer)).toString('hex');
        ...
        //console.log("job.rpcData:", job.rpcData.uscData);
        _this.emit('share', {
            job: jobId,
            ip: ipAddress,
            port: port,
	        ...
	        coinbaseTxHex: job.coinbaseTx,
	        isUscTarget: isUscTarget,
	        isDuplicate: job.rpcData.isDuplicate,	
	        merkleHashes: job.merkleBranch,
	        headerBuffer: headerBuffer.toString('hex'),
            blockHash: blockHash,
            blockHashInvalid: blockHashInvalid,
            cTotalTxCountHex: job.cTotalTxCountHex
        }, blockHex);
