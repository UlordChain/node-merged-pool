...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNonce, 
   ...
   */
	...
	if(rpcData.uscData){
		this.uscTarget = bignum(rpcData.uscData.target.substr(2), 16);
		this.uscDifficulty = util.numToString(parseFloat((diff1 / this.uscTarget.toNumber()).toFixed(9)));
	}
    ...
    if(rpcData.transactions.length > 0) {
        txsWithCoinbase = txsWithCoinbase.concat(getTransactionBuffers(rpcData.transactions));
        txsWithCoinbase[0] = this.coinbaseTxHash;
    } else {
        txsWithCoinbase = [this.coinbaseTxHash]
    }

    this.cTotalTxCountHex = txsWithCoinbase.length.toString(16);
    var cMerkleTree = new merkleTree(txsWithCoinbase);

    if(cMerkleTree.steps.length > 0) {
        this.merkleBranch = [this.coinbaseTxHash.toString("hex")].concat(getMerkleHashes(cMerkleTree.steps));
    }
    else {
        this.merkleBranch = [this.coinbaseTxHash.toString("hex")]; 
    }
    
    this.merkleBranch = this.merkleBranch.join(" ");
	...
	this.serializeawHeader = function(){
