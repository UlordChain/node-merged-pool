...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
	...
	/* pay to posnode */
    if (rpcData.posnode && rpcData.posnode.pospayee) {
        var payeeReward = rpcData.posnode.posamount;
        reward -= payeeReward;
        rewardToPool -= payeeReward;

        var payeeScript = util.addressToScript(rpcData.posnode.pospayee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            util.varIntBuffer(payeeScript.length),
            payeeScript
        ]));
     console.log("pay to posnode: " + payeeScript.toString('hex') + " " + payeeReward + ", in height:" + rpcData.height);
    }
    ...
    //USCTag: "USCBLOCK:blockHashForMergedMining"
    //console.log('uscData==============',rpcData.uscData);
    if(rpcData.isUscData && rpcData.uscData != undefined){
        var blockHashForMergedMining = rpcData.uscData.blockHashForMergedMining;
	txOutputBuffers.push(Buffer.concat([
		util.packInt64LE(0),
		util.varIntBuffer(util.getUscTag(blockHashForMergedMining).length),
		util.getUscTag(blockHashForMergedMining)
	]));
    }
...
exports.createGeneration = function(rpcData, publicKey, reward, txMessages, recipients){
    ...
    var txComment = txMessages === true ?
        util.serializeString('http://www.u1pool.com') :
        new Buffer([]);
    ...
    var scriptSigPart2 = util.serializeString('/u1pool.com/');
